...
{
    // vars
    public $url = 'edit.php?post_type=acf-field-group',
        $sync = array();
    ...
    */
    ...
    public function render_column($column, $field_group)
    {
...
<div class="acf-column-2">
	<div class="acf-box">
		<div class="inner">
			<h2><?php echo acf_get_setting('name');
        ?></h2>

			<h3><?php _e('Changelog', 'acf');
        ?></h3>
			<p><?php _e("See what's new in", 'acf');
        ?> <a href="<?php echo admin_url('edit.php?post_type=acf-field-group&page=acf-settings-info&tab=changelog');
        ?>"><?php _e('version', 'acf');
        ?> <?php echo acf_get_setting('version');
        ?></a>
			
			<h3><?php _e('Resources', 'acf');
        ?></h3>
			<ul>
				<li><a href="<?php echo $www;
        ?>#getting-started" target="_blank"><?php _e('Getting Started', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#updates" target="_blank"><?php _e('Updates', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#field-types" target="_blank"><?php _e('Field Types', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#functions" target="_blank"><?php _e('Functions', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#actions" target="_blank"><?php _e('Actions', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#filters" target="_blank"><?php _e('Filters', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#how-to" target="_blank"><?php _e("'How to' guides", 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#tutorials" target="_blank"><?php _e('Tutorials', 'acf');
        ?></a></li>
				<li><a href="<?php echo $www;
        ?>#faq" target="_blank"><?php _e('FAQ', 'acf');
        ?></a></li>
			</ul>
		</div>
		<div class="footer footer-blue">
			<ul class="acf-hl">
				<li><?php _e('Created by', 'acf');
        ?> Elliot Condon</li>
			</ul>
		</div>
	</div>
...
(function($){
	...
	$('#the-list tr').each(function(){
		...
		if( id ) {
			...
			// create el
			var $span = $('<span class="acf-duplicate-field-group"><a title="<?php _e('Duplicate this item', 'acf');
        ?>" href="<?php echo admin_url($this->url.'&acfduplicate=');
        ?>' + post_id + '&_wpnonce=<?php echo wp_create_nonce('bulk-posts');
        ?>"><?php _e('Duplicate', 'acf');
        ?></a> | </span>');
...
<?php
    ...
    */
    ...
    public function sync_admin_footer()
    {
...
<?php foreach ($this->sync as $field_group): $i++;
	...
	<tr <?php if ($i % 2 == 0): ?>class="alternate"<?php endif;
		...
		<th class="check-column" scope="row">
			<label for="cb-select-<?php echo $field_group['key'];
        ?>" class="screen-reader-text"><?php printf(__('Select %s', 'acf'), $field_group['title']);
        ?></label>
			<input type="checkbox" value="<?php echo $field_group['key'];
        ?>" name="post[]" id="cb-select-<?php echo $field_group['key'];
        ?>">
		</th>
		<td class="post-title page-title column-title">
			<strong>
				<span class="row-title"><?php echo $field_group['title'];
        ?></span><span class="acf-description"><?php echo $field_group['key'];
        ?>.json</span>
			</strong>
			<div class="row-actions">
				<span class="import"><a title="<?php echo esc_attr(__('Synchronise field group', 'acf'));
        ?>" href="<?php echo admin_url($this->url.'&post_status=sync&acfsync='.$field_group['key'].'&_wpnonce='.wp_create_nonce('bulk-posts'));
        ?>"><?php _e('Sync', 'acf');
        ?></a></span>
			</div>
		</td>
        ...
        ?>"><?php $this->render_column($column, $field_group);
        ?></td>
		<?php endforeach;
        ?>
	</tr>
