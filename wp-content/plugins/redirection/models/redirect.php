...
class Red_Item
{
    ...
    public function __construct($values, $type = '', $match = '')
    {
        if (is_object($values)) {
    ...
    public static function get_for_url($url, $type)
    {
        global $wpdb;
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                $items[] = array('position' => ($row->group_pos * 1000) + $row->position, 'item' => new self($row));
            }
        }
    ...
    public static function get_by_module($module)
    {
        global $wpdb;
    ...
    public static function sanitize_url($url, $regex)
    {
    ...
    public static function save_order($items, $start)
    {
        global $wpdb;
